...
Class Usuario
{
	...
	public function guardar($resultado, $fecha, $hora, $accion){
		$sql="INSERT INTO cronograma (idprender,fecha,hora,accion)
		VALUES ('$resultado','$fecha','$hora','$accion')";
		return ejecutarConsulta($sql);
	}
	...
	//Implementamos un método para desactivar categorías
	public function prende($led)
	{
		$sql="UPDATE prender SET prender='1' WHERE id='$led'";
		return ejecutarConsulta($sql);
	}

	//Implementamos un método para desactivar categorías
	public function apaga($led)
	{
		$sql="UPDATE prender SET prender='0' WHERE id='$led'";
		return ejecutarConsulta($sql);
	}

	public function listarfocos()
	{
		$sql="SELECT * FROM prender";
		return ejecutarConsulta($sql);		
	}

	//Implementar un método para listar los permisos marcados
	public function listarfocosmarcados()
	{
		$sql="SELECT * FROM prender WHERE prender='1'";
		return ejecutarConsulta($sql);
	}

	public function consultar()
	{
		$sql="SELECT * FROM cronograma WHERE CONCAT(fecha, ' ' ,hora) < DATE_ADD(NOW(),INTERVAL 1 MINUTE) AND CONCAT(fecha, ' ' ,hora) > DATE_SUB(NOW(),INTERVAL 1 MINUTE) AND activo = 1;";
		return ejecutarConsulta($sql);
	}	

	public function desactivarcrono($idusuario)
	{
		$sql="UPDATE cronograma SET activo='0' WHERE idprograma='$idusuario'";
		return ejecutarConsulta($sql);
	}	
